...
export const getSprints = (dispatch, projectId) => {
    axios.get(`/apiprojects/projects/${projectId}/sprints`)
    .then(res=>res.data)
    .then(sprints=>{dispatch(fetchedSprints(sprints))})
    return {
        type: "FETCH_SPRINTS"
    }
}
...
export const addSprint = (dispatch,projectId, newSprint) => {
    axios.post(`/apiprojects/projects/${projectId}/sprints`,
    newSprint)
    .then(res=>res.data)
...
export const deleteSprint=(dispatch,currentProjectId,sprintId)=>{
    axios.delete(`/apiprojects/projects/${currentProjectId}/sprints/${sprintId}`)
    .then(res=>{
        if(res.status===200){
